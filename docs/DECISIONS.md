...
- Consequence: Node Library load and mode switches, workspace double-click focus, the single-node `Center` CTA, and multi-node `Center Selection` now share one union-bounds viewport target calculation; deterministic shells use authored target sizes directly, content-fit shells preflight-measure a hidden copy before the visible move begins, and measured outer shell bounds only validate that prediction inside the same motion window instead of issuing a second visible recenter later.
